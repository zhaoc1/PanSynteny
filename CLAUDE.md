...
**Project status.** v0.5.0 testing release published to `github.com/zhaoc1/PanSynteny` (private). Schema may shift between 0.x.y versions before 1.0.
...
pipeline.R Step 1    per-focal neighbor TSVs + catalog c80 tables
           ...
           Step 6    canonical_paths_c80s      -->  representative_path.tsv + rep.tsv (block reps x genome)
```
...
- **L3 per-genome** - one row per `(canonical, contributing genome)`. Only level with per-genome `gene_id` resolution. Carries `needs_flip` (boolean computed once at the canonical x collapsed-path grain) and a `gene_path_canonical` already flipped according to it.
...
- **`needs_flip` lives at the canonical x collapsed-path grain.** Per-genome rows inherit it; L1 / L2 cannot promote it because mirror-image collapsed groups roll up. Don't try to add `needs_flip` to the L1 or L2 tables.
...
- **`cfg_get(job_config, "key")` for every tunable.** No `cfg_get`-bypassing constants in helper files. The full set of YAML keys + defaults is enumerated in [USER_GUIDE.md section Tunables](docs/USER_GUIDE.md).
