...
| --- | --- | --- | --- |
| `is_truncated` | logical | per row | This gene shorter than `cutoff x centroid_80_gene_length`? |
...
**(canonical x collapsed_path)** grain in `explode_canonical_into_collapsed_paths`
...
Two parallel "exploded" tables, one row per `(canonical path x c80 position)`:
...
- `map_representatives_to_genomes()` - link each representative back to the
  genomes it appears in. Outputs `rep_slim` (block_uid x neighbor_genome).
